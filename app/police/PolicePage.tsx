...
export default function PolicePage() {
  ...
  const { officers, zones, hazards, confirmHazard, resolveHazard, addHazard, forceSeed } = store;

  // Ensure officers are loaded
  useEffect(() => {
                ...
                return (
                  <Marker key={h.id} position={[h.lat, h.lng]} icon={icon}>
                    <Popup>
                      ...
                      <span className="text-xs text-slate-400">{h.status} · {h.reportedBy === 'officer' ? '👮 Officer' : '👤 Citizen'}</span>
                    </Popup>
                  </Marker>
                );
              })}
          ...
          {selectedZone && (
            <button
              ...
              className={`absolute top-4 right-4 z-[1000] flex items-center gap-2 px-4 py-2 rounded-full text-sm font-medium transition ${
                flagMode ? 'bg-red-600 text-white hover:bg-red-700' : 'bg-slate-800/80 text-slate-300 hover:bg-slate-700 border border-slate-600'
              }`}
            >
                ...
                unconfirmedHazards.length === 0 ? (
                  ...
                  unconfirmedHazards.map(h => (
                    <div key={h.id} className="bg-slate-800/40 rounded-lg p-3 border border-slate-700/30">
                      ...
                      <div className="mt-1 text-xs text-slate-500">Weight: {h.weight} {h.weight >= 3 ? '✅ Auto-activated' : ''}</div>
                    </div>
                  ))
                )
              ) : (
